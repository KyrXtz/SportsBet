...
public class WebhookBinder : IModelBinder
{
    ...
    private Webhook? CommandFactory(string actionName, string? contentType, string payload)
    {
        switch (actionName)
        {
            ...
            case Constants.Methods.SportsTickerEvent:
                object? eventListReturnedObj = contentType switch
                {
                    Constants.ContentTypes.Json => payload.FromJson<SportsTickerEventListJsonRequest>(),
                    Constants.ContentTypes.Xml => payload.FromXml<SportsTickerEventListXmlRequest>(),
                    _ => null
                };

                if (eventListReturnedObj is null) break;

                var eventListPayload = _mapper.Map<SportsTickersEventsPayload>(eventListReturnedObj);
                return new SportsTickersEventsCommand(eventListPayload);
        }
    ...
    private string GetActionEvent(string request)
    {
        return request.Contains("keep_alive") ? Constants.Methods.KeepAliveEvent :
            ...
            request.Contains("event_list") ? Constants.Methods.SportsTickerEvent :
